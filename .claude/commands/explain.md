...
1. **Role**: What this file does (from node `type` field)
2. **Group**: Which architectural layer/domain it belongs to
3. **Importance**: core / supporting / peripheral
4. **Dependencies** (outgoing edges): What this file uses
5. **Dependents** (incoming edges): What uses this file
6. **Cross-group connections**: Any edges that cross group boundaries

Keep the explanation concise (under 15 lines). Use the file's architectural role, not just its name.

If no mind map exists, tell the user to run `/mindmap` first.
